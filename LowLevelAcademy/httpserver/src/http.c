  ...
  while (line_start && *line_start && *line_start != '\r' &&
    ...
    if (colon) {
      *colon = '\0';
      ...
      request->headers = (http_header_t *)realloc(request->headers,
...
void add_http_header(http_response *response, const char *key,
  ...
  response->headers[response->header_count]
      .key[sizeof(response->headers[response->header_count].key) - 1] = '\0';
  strncpy(response->headers[response->header_count].value, value,
          sizeof(response->headers[response->header_count].value) - 1);
  response->headers[response->header_count]
      .value[sizeof(response->headers[response->header_count].value) - 1] =
      '\0';
